...
async function _proxyThen(options: ProxyThenRunnerOptions): Promise<unknown> {
  ...
  const { builderPostProcessors } = kmore
  ...
  try {
    if (builderType !== KmoreBuilderType.counter && Array.isArray(builderPostProcessors)) {
      for (const processor of builderPostProcessors) {
        assert(typeof processor === 'function', 'builderPostProcessors should be an array of functions')
        // eslint-disable-next-line no-await-in-loop
        builder = (await processor({ kmore, builder })).builder
      }
    }
