...
export class UserService {
  ...
  async userUpdateDel(
    ...
    assert(total0 === expectTotal, `total0: ${total0} vs expect: ${expectTotal}`)
    ...
    const realName = 'test' + Math.random().toString()
    const user2 = await this.updateUser(uid, realName)
    assert(user2.realName === realName, `${user2.realName} vs expect: ${realName}`)
  ...
  async selfMissingAwait(
    ...
    console.log('selfMissingAwait', { uid, expectTotal, expectTotalNoTrx })
  }
