...
export class UserService {
  ...
  async userUpdateDel(
    ...
    assert(total0 === expectTotal, `total0:${total0} != expectTotal:${expectTotal}`)
    ...
    user3.forEach((row) => {
      if (row.uid === uid) {
        assert(row.realName === realName, `row.realName:${row.realName} != realName:${realName}`)
      }
    })
    assert(trx0 === trx3)
    ...
    const expectTotal2 = expectTotal - 1
    assert(total2 === expectTotal2, `total2:${total2} != expectTotal2:${expectTotal2}`)
    assert(trx0 === trx2)
    ...
    const expectTotal6 = expectTotal - 1
    assert(user6.length === expectTotal6, `user6.length:${user6.length} != expectTotal6:${expectTotal6}`)
    assert(trx0 === trx6)
  }
